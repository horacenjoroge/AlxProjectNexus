...
def validate_geographic_restriction(
    ...
    if country:
        ...
        if blocked_countries and country.upper() in [
        ...
        if allowed_countries:
            if country.upper() not in [c.upper() for c in allowed_countries]:
                return (
                    False,
                    f"Voting is only allowed from: {', '.join(allowed_countries)}",
                )
    ...
    if region:
        ...
        if allowed_regions:
            if region.upper() not in [r.upper() for r in allowed_regions]:
                return (
                    False,
                    f"Voting is only allowed from regions: {', '.join(allowed_regions)}",
                )
