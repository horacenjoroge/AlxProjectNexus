...
def is_ip_blocked(ip_address: str) -> Tuple[bool, Optional[str]]:
    ...
    block = IPBlock.objects.filter(ip_address=ip_address, is_active=True).first()
...
def record_ip_violation(
    ...
    try:
        ...
        if auto_block:
            threshold = getattr(
                settings, "IP_VIOLATION_THRESHOLD", DEFAULT_VIOLATION_THRESHOLD
            )
            reputation_threshold = getattr(
                settings, "IP_REPUTATION_THRESHOLD", DEFAULT_REPUTATION_THRESHOLD
            )

            should_block = (
                reputation.violation_count >= threshold
                or reputation.reputation_score < reputation_threshold
            )
            ...
            if should_block:
                ...
                if not is_ip_blocked(ip_address)[0]:
                    return block_ip(
                        ...
                        is_manual=False,
                        auto_unblock_hours=getattr(
                            settings,
                            "IP_AUTO_UNBLOCK_HOURS",
                            DEFAULT_AUTO_UNBLOCK_HOURS,
                        ),
                    )
...
def block_ip(
    ...
    IPBlock.objects.filter(ip_address=ip_address, is_active=False).update(
        is_active=False
    )
