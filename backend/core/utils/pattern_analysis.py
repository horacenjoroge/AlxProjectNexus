...
def detect_single_ip_single_option_pattern(
    ...
    ).values("ip_address", "option_id").annotate(
        vote_count=Count("id")
    ).order_by("-vote_count")
...
def detect_time_clustered_votes(
    ...
    votes = Vote.objects.filter(
        poll_id=poll_id,
        created_at__gte=cutoff,
    ).order_by("created_at").values("id", "created_at", "ip_address")
        ...
        if not current_cluster:
            ...
            if time_diff <= cluster_window_seconds:
                ...
                if len(current_cluster) >= min_votes_in_cluster:
                    unique_ips = len(set(v["ip_address"] for v in current_cluster if v["ip_address"]))
...
def detect_geographic_anomalies(
    ...
    votes = Vote.objects.filter(
        ...
        created_at__gte=cutoff,
        ip_address__isnull=False,
    ).order_by("ip_address", "created_at").values(
        "id", "ip_address", "created_at", "user_id"
    )
    ...
    for user_id, user_vote_list in user_votes.items():
        ...
        for i in range(1, len(user_vote_list)):
            ...
            time_diff = (curr_vote["created_at"] - prev_vote["created_at"]).total_seconds()
            
            # If IP changed and time difference is very short (< 1 minute)
            if (prev_vote["ip_address"] != curr_vote["ip_address"] and 
                time_diff < 60):
                anomalies.append({
    ...
    if vpn_proxy_ips:
        ...
        for vote in votes:
            ...
            if any(ip.startswith(prefix) for prefix in vpn_proxy_ips if isinstance(prefix, str)):
                anomalies.append({
                    "ip_address": ip,
...
def detect_user_agent_anomalies(
    ...
    ).exclude(user_agent="").values("user_agent", "user_id", "ip_address")
...
def analyze_vote_patterns(
    ...
    highest_risk = max(
        (p.get("risk_score", 0) for p in all_pattern_list),
        default=0
    )
