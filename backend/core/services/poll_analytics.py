...
def get_votes_by_hour(poll_id: int, date: Optional[datetime] = None) -> List[Dict]:
    ...
    for item in hourly_counts:
        hour = item["hour"].hour if hasattr(item["hour"], "hour") else item["hour"].time().hour
...
def get_voter_demographics(poll_id: int) -> Dict:
    ...
    authenticated_count = votes.exclude(user__isnull=True).values("user").distinct().count()
    anonymous_count = votes.filter(user__isnull=True).count()

    # Unique IPs
    unique_ips = votes.exclude(ip_address__isnull=True).values("ip_address").distinct().count()
...
def get_participation_rate(poll_id: int) -> Dict:
    ...
    try:
        ...
        return {"participation_rate": 0, "unique_voters": 0, "total_potential_voters": 0}

    # Get unique voters
    unique_voters = Vote.objects.filter(
        poll_id=poll_id, is_valid=True
    ).values("user", "voter_token").distinct().count()
...
def get_drop_off_rate(poll_id: int) -> Dict:
    ...
    failed_attempts = VoteAttempt.objects.filter(
        poll_id=poll_id, success=False
    ).count()
...
def get_analytics_summary(poll_id: int) -> Dict:
    ...
    return {
        ...
        "top_option": max(distribution, key=lambda x: x["vote_count"]) if distribution else None,
