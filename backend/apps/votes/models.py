...
class Vote(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="votes", null=True, blank=True
    )
    ...
    voter_token = models.CharField(
        max_length=64, db_index=True, help_text="Token for anonymous/guest voting"
    )
    # Idempotency
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, db_index=True, help_text="IP address of voter"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            ),  # For poll + voter_token lookups
            models.Index(
                fields=["idempotency_key"]
            ),  # Already unique, but explicit index
            models.Index(
                fields=["ip_address", "created_at"]
            ),  # For IP + timestamp queries
...
class VoteAttempt(models.Model):
    ...
    user = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="vote_attempts",
    )
    poll = models.ForeignKey(
        Poll, on_delete=models.CASCADE, related_name="vote_attempts"
    )
    option = models.ForeignKey(
        PollOption,
        on_delete=models.SET_NULL,
    ...
    voter_token = models.CharField(
        ...
        db_index=True,
        blank=True,
        help_text="Token for anonymous/guest voting",
    )
    # Idempotency
    idempotency_key = models.CharField(
        max_length=64, db_index=True, help_text="Idempotency key used in attempt"
    )
    # Tracking fields
    ...
    success = models.BooleanField(
        default=False, help_text="Whether the vote attempt was successful"
    )
    error_message = models.TextField(
        blank=True, help_text="Error message if attempt failed"
    )
    # Timestamps
    created_at = models.DateTimeField(auto_now_add=True)
