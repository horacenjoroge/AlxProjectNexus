...
class TestAPIDocumentation:
    ...
    def test_schema_endpoints_documented(self):
        ...
        assert "/api/v1/polls/" in paths or any(
            "/polls" in path for path in paths.keys()
        )
        assert "/api/v1/votes/" in paths or any(
            "/votes" in path for path in paths.keys()
        )
        assert "/api/v1/users/" in paths or any(
            "/users" in path for path in paths.keys()
        )
        assert "/api/v1/analytics/" in paths or any(
            "/analytics" in path for path in paths.keys()
        )
    ...
    def test_idempotency_documented(self):
        ...
        if vote_path:
            ...
            assert (
                "idempotency" in description.lower()
                or "idempotent" in description.lower()
            )
