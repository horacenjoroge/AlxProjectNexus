...
class TestDeploymentGuide:
    ...
    def test_environment_variables_documented(self, guide_content):
        ...
        assert not missing_vars, f"Missing environment variables in documentation: {missing_vars}"
    ...
    def test_docker_commands_documented(self, guide_content):
        ...
        assert not missing_patterns, f"Missing Docker command patterns: {missing_patterns}"
    ...
    def test_monitoring_setup_documented(self, guide_content):
        ...
        found_topics = [topic for topic in monitoring_topics if topic.lower() in guide_content.lower()]
        
        assert len(found_topics) >= 3, f"Insufficient monitoring documentation. Found: {found_topics}"
    ...
    def test_troubleshooting_section_exists(self, guide_content):
        ...
        assert not missing_topics, f"Troubleshooting section incomplete. Missing: {missing_topics}"
    ...
    def test_code_references_exist(self, guide_content):
        ...
        for ref in references:
            ...
            if "/" in ref and (ref.startswith("docker/") or ref.startswith("backend/") or ref.startswith("scripts/")):
                ...
                if not full_path.exists() and not any(opt in file_path for opt in ["docker-compose.prod.yml", "nginx-ssl.conf"]):
    ...
    def test_docker_compose_prod_referenced(self, guide_content):
        ...
        assert "docker-compose.prod.yml" in guide_content or "docker-compose.prod" in guide_content, \
    ...
    def test_environment_example_provided(self, guide_content):
        ...
        assert "ALLOWED_HOSTS=" in guide_content, "ALLOWED_HOSTS example should be provided"
    ...
    def test_ssl_certificate_renewal_documented(self, guide_content):
        ...
        found_topics = [topic for topic in renewal_topics if topic.lower() in guide_content.lower()]
        
        assert len(found_topics) >= 2, f"SSL renewal not fully documented. Found: {found_topics}"

    def test_backup_restore_procedures_documented(self, guide_content):
        """Test that backup and restore procedures are documented."""
        assert "backup" in guide_content.lower(), "Backup procedures should be documented"
        assert "restore" in guide_content.lower(), "Restore procedures should be documented"
    ...
    def test_health_check_endpoints_documented(self, guide_content):
        ...
        assert len(found_checks) >= 1, f"Health check endpoints not documented. Found: {found_checks}"
    ...
    def test_post_deployment_verification_documented(self, guide_content):
        ...
        verification_section = guide_content.split("## 8. Post-Deployment Verification")[1].split("## 9.")[0]
        
        assert "curl" in verification_section or "docker-compose" in verification_section, \
    ...
    def test_production_checklist_exists(self, guide_content):
        ...
        assert "Production Checklist" in guide_content or "checklist" in guide_content.lower(), \
    ...
    def test_geographic_restrictions_documented(self, guide_content):
        ...
        assert "KE" in guide_content or "Kenya" in guide_content, \
    ...
    def test_documentation_structure(self, guide_content):
        ...
        assert "## Table of Contents" in guide_content, "Table of contents should be present"
        
        # Check for code blocks
        code_blocks = guide_content.count("```")
        assert code_blocks >= 20, f"Documentation should have code examples. Found {code_blocks // 2} blocks"

    def test_quick_reference_section(self, guide_content):
        """Test that quick reference section exists."""
        assert "Quick Reference" in guide_content or "Appendix" in guide_content, \
    ...
    def test_all_steps_have_commands(self, guide_content):
        ...
        for section in sections_with_commands:
            section_content = guide_content.split(f"## {section.split()[0]}")[1].split("##")[0] if f"## {section.split()[0]}" in guide_content else ""
            if section_content:
                assert "```" in section_content, f"Section '{section}' should have code examples"
