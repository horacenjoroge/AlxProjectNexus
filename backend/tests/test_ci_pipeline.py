...
class TestCIPipelineConfiguration:
    ...
    def test_ci_workflow_triggers_on_pr(self, ci_workflow):
        ...
        assert (
            "pull_request" in on_config
        ), f"Should have pull_request trigger. Found: {list(on_config.keys())}"
    ...
    def test_ci_workflow_has_lint_job(self, ci_workflow):
        ...
        assert any(
            "isort" in name.lower() for name in step_names
        ), "Should have isort check"
        assert any(
            "flake8" in name.lower() for name in step_names
        ), "Should have flake8 check"
        assert any(
            "Bandit" in name or "bandit" in name.lower() for name in step_names
        ), "Should have Bandit security scan"
    ...
    def test_ci_workflow_has_test_job(self, ci_workflow):
        ...
        test_job = jobs["test"]
        assert (
            "services" in test_job
        ), "Test job should have services (PostgreSQL, Redis)"
    ...
    def test_lint_job_fails_on_errors(self, ci_workflow):
        ...
        for step in steps:
            ...
            if "Black" in step_name:
                black_found = True
                assert (
                    "exit 1" in run_cmd or "||" in run_cmd
                ), f"Black check should fail on errors. Command: {run_cmd[:200]}"
            if "isort" in step_name.lower():
                isort_found = True
                assert (
                    "exit 1" in run_cmd or "||" in run_cmd
                ), f"isort check should fail on errors. Command: {run_cmd[:200]}"
            if (
                "Bandit" in step_name or "bandit" in step_name.lower()
            ) and "Security scan" in step_name:
                bandit_found = True
    ...
    def test_staging_deployment_triggers_on_develop(self, deploy_staging_workflow):
        ...
        # YAML may have True as key due to GitHub Actions expressions
        on_config = (
            deploy_staging_workflow.get("on") or deploy_staging_workflow.get(True) or {}
        )
        assert (
            "push" in on_config
        ), f"Should have push trigger. Found: {list(on_config.keys())}"
        push_config = on_config["push"]
        branches = push_config.get("branches", [])
        assert (
            "develop" in branches or isinstance(branches, str) and "develop" in branches
        )
    ...
    def test_production_deployment_triggers_on_main(self, deploy_production_workflow):
        ...
        # YAML may have True as key due to GitHub Actions expressions
        on_config = (
            deploy_production_workflow.get("on")
            or deploy_production_workflow.get(True)
            or {}
        )
        assert (
            "push" in on_config
    ...
    def test_docker_build_in_ci(self, ci_workflow):
        ...
        docker_steps = [
            s
            for s in steps
            if "docker" in s.get("name", "").lower() or "Docker" in s.get("name", "")
        ]
    ...
    def test_docker_registry_push_configured(self, ci_workflow):
        ...
        for step in steps:
            if (
                "login" in step.get("name", "").lower()
                or "registry" in step.get("name", "").lower()
            ):
                login_step = step
                break

        # Login step should exist (may be conditional on PR)
        assert login_step is not None or any(
            "push" in str(step) for step in steps
        ), "Should have registry login or push step"
    ...
    def test_bandit_configured(self):
        ...
        ), "Bandit configuration file (.bandit) should exist"

    def test_bandit_in_requirements(self):
        """Test that Bandit is in requirements."""
        dev_requirements = (
            Path(__file__).parent.parent.parent / "requirements" / "development.txt"
        )
        assert dev_requirements.exists()

        content = dev_requirements.read_text()
        assert (
            "bandit" in content.lower()
        ), "Bandit should be in development requirements"
    ...
    def test_pr_check_job_exists(self, ci_workflow):
        ...
        assert (
            "pr-check" in jobs
            or "pr_check" in jobs
            or any("pr" in job.lower() for job in jobs.keys())
...
class TestCIPipelineBehavior:
    ...
    def test_deployment_only_after_ci_passes(self):
        ...
        assert (
            True  # Configuration verified in test_staging_deployment_requires_ci_checks
        )
